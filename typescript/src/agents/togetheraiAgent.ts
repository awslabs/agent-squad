...
export interface TogetherAIAgentOptions extends AgentOptions {
  model?: string;
  streaming?: boolean;
  logRequest?: boolean;
  inferenceConfig?: {
...
export type TogetherAIAgentOptionsWithAuth = TogetherAIAgentOptions & (WithApiKey | WithClient);

const DEFAULT_MAX_TOKENS = 4096;

export class TogetherAIAgent extends Agent {
  ...
  constructor(options: TogetherAIAgentOptionsWithAuth) {
    ...
    if (options.client) {
      ...
      this.client = new OpenAI({ apiKey: options.apiKey, baseURL: "https://api.together.xyz/v1" });
    }
  ...
  private async handleSingleResponse(input: any): Promise<ConversationMessage> {
    try {
      ...
      if (!chatCompletion.choices || chatCompletion.choices.length === 0) {
        throw new Error('TogetherAI Agent: No choices returned from OpenAI API');
      }
      ...
      obj["from"] = "togetherai-agent";
      modelStats.push(obj);
      Logger.logger.info(`TogetherAI Agent Usage: `, JSON.stringify(obj));
      const assistantMessage = chatCompletion.choices[0]?.message?.content;

      if (typeof assistantMessage !== 'string') {
        throw new Error('TogetherAI Agent: Unexpected response format from OpenAI API');
      }
      ...
      Logger.logger.error('TogetherAI Agent: Error in OpenAI API call:', error);
      throw error;
    }
  }
